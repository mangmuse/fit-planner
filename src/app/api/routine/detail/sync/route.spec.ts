jest.mock("@/app/api/_utils/handleError", () => ({
  ...jest.requireActual("@/app/api/_utils/handleError"),
  handleServerError: jest.fn(),
}));
jest.mock("@/util/validateData");

import { mockRoutineDetail } from "@/__mocks__/routineDetail.mock";
import { handleServerError, HttpError } from "@/app/api/_utils/handleError";
import { POST } from "@/app/api/routine/detail/sync/route";
import { prisma } from "@/lib/prisma";

import { validateData } from "@/util/validateData";
import { NextRequest, NextResponse } from "next/server";
import { RoutineDetail, Routine } from "@prisma/client";
import { createPrismaRoutineDetailResponse } from "@/__mocks__/app/api/routine/detail/mockData";

describe("POST /api/routine/detail/sync", () => {
  const mockedHandleServerError = handleServerError as jest.Mock;
  const mockedValidateData = validateData as jest.Mock;
  const mockedPrismaRoutineDetailCreate = prisma.routineDetail
    .create as jest.Mock;
  const mockedPrismaRoutineDetailUpdate = prisma.routineDetail
    .update as jest.Mock;
  const mockedPrismaRoutineFindUnique = prisma.routine.findUnique as jest.Mock;

  beforeEach(() => {
    jest.clearAllMocks();
  });

  const targetUrl = "http://localhost/api/routine/detail/sync";
  const mockUnsyncedWithServerId = {
    ...mockRoutineDetail.unsynced,
    serverId: "mock-server-detail-id",
    routineId: "mock-routine-id",
  };
  const mockUnsyncedWithoutServerId = {
    ...mockRoutineDetail.unsynced,
    serverId: null,
    routineId: "mock-routine-id",
  };
  const mockRoutineResponse: Routine = {
    id: "mock-routine-id",
    userId: "user-123",
    name: "테스트루틴",
    description: null,
    createdAt: new Date(),
    updatedAt: new Date(),
  };
  const mockRoutineDetailResponse: RoutineDetail =
    createPrismaRoutineDetailResponse();

  it("serverId가 있는 경우 업데이트하고 updatedList를 반환한다", async () => {
    const req = new NextRequest(targetUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ mappedUnsynced: [mockUnsyncedWithServerId] }),
    });

    mockedValidateData.mockReturnValue({
      mappedUnsynced: [mockUnsyncedWithServerId],
    });
    mockedPrismaRoutineDetailUpdate.mockResolvedValue(
      mockRoutineDetailResponse
    );
    mockedPrismaRoutineFindUnique.mockResolvedValue(mockRoutineResponse);

    const res = await POST(req);
    const body = await res.json();

    expect(res.status).toBe(200);
    expect(body.success).toBe(true);
    expect(mockedPrismaRoutineDetailCreate).not.toHaveBeenCalled();
    expect(mockedPrismaRoutineDetailUpdate).toHaveBeenCalledWith({
      where: { id: mockUnsyncedWithServerId.serverId },
      data: expect.objectContaining({
        weight: mockUnsyncedWithServerId.weight,
        setOrder: mockUnsyncedWithServerId.setOrder,
        rpe: mockUnsyncedWithServerId.rpe,
      }),
    });
    expect(body.updated).toEqual([
      {
        localId: mockUnsyncedWithServerId.id,
        serverId: mockRoutineDetailResponse.id,
        exerciseId: mockUnsyncedWithServerId.exerciseId,
        routineId: mockUnsyncedWithServerId.routineId,
      },
    ]);
  });

  it("serverId가 없는 경우 새로 생성하고 updatedList를 반환한다", async () => {
    const req = new NextRequest(targetUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ mappedUnsynced: [mockUnsyncedWithoutServerId] }),
    });

    mockedValidateData.mockReturnValue({
      mappedUnsynced: [mockUnsyncedWithoutServerId],
    });
    mockedPrismaRoutineFindUnique.mockResolvedValue(mockRoutineResponse);
    mockedPrismaRoutineDetailCreate.mockResolvedValue(
      mockRoutineDetailResponse
    );

    const res = await POST(req);
    const body = await res.json();

    expect(res.status).toBe(200);
    expect(body.success).toBe(true);
    expect(mockedPrismaRoutineDetailCreate).toHaveBeenCalledWith({
      data: expect.objectContaining({
        routine: { connect: { id: mockUnsyncedWithoutServerId.routineId } },
        exercise: { connect: { id: mockUnsyncedWithoutServerId.exerciseId } },
        weight: mockUnsyncedWithoutServerId.weight,
        setOrder: mockUnsyncedWithoutServerId.setOrder,
      }),
    });
    expect(body.updated).toEqual([
      {
        localId: mockUnsyncedWithoutServerId.id,
        serverId: mockRoutineDetailResponse.id,
        exerciseId: mockUnsyncedWithoutServerId.exerciseId,
        routineId: mockUnsyncedWithoutServerId.routineId,
      },
    ]);
  });

  it("routine이 존재하지 않으면 404 에러를 반환한다", async () => {
    const req = new NextRequest(targetUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ mappedUnsynced: [mockUnsyncedWithoutServerId] }),
    });

    mockedValidateData.mockReturnValue({
      mappedUnsynced: [mockUnsyncedWithoutServerId],
    });
    mockedPrismaRoutineFindUnique.mockResolvedValue(null);
    mockedHandleServerError.mockReturnValue(
      NextResponse.json(
        {
          success: false,
          message: "RoutineId가 일치하는 routine을 찾지 못했습니다",
        },
        { status: 404 }
      )
    );

    const res = await POST(req);
    const body = await res.json();

    expect(res.status).toBe(404);
    expect(body.success).toBe(false);
    expect(body.message).toBe("RoutineId가 일치하는 routine을 찾지 못했습니다");
    expect(mockedPrismaRoutineDetailCreate).not.toHaveBeenCalled();
    expect(mockedHandleServerError).toHaveBeenCalled();
  });

  it("mappedUnsynced가 빈 배열인 경우 빈 updatedList를 반환한다", async () => {
    const req = new NextRequest(targetUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ mappedUnsynced: [] }),
    });

    mockedValidateData.mockReturnValue({ mappedUnsynced: [] });

    const res = await POST(req);
    const body = await res.json();

    expect(res.status).toBe(200);
    expect(body.success).toBe(true);
    expect(body.updated).toEqual([]);
  });

  it("validation이 실패하면 422 에러를 반환한다", async () => {
    const req = new NextRequest(targetUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ mappedUnsynced: "invalid" }),
    });

    const mockError = new HttpError("잘못된 요청입니다", 422);
    mockedValidateData.mockImplementation(() => {
      throw mockError;
    });
    mockedHandleServerError.mockReturnValue(
      NextResponse.json(
        { success: false, message: mockError.message },
        { status: 422 }
      )
    );

    const res = await POST(req);
    const body = await res.json();

    expect(res.status).toBe(422);
    expect(body.success).toBe(false);
    expect(body.message).toBe("잘못된 요청입니다");
    expect(mockedHandleServerError).toHaveBeenCalledWith(mockError);
    expect(mockedPrismaRoutineDetailCreate).not.toHaveBeenCalled();
    expect(mockedPrismaRoutineDetailUpdate).not.toHaveBeenCalled();
  });

  it("데이터베이스 에러가 발생하면 500 에러를 반환한다", async () => {
    const req = new NextRequest(targetUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ mappedUnsynced: [mockUnsyncedWithServerId] }),
    });

    mockedValidateData.mockReturnValue({
      mappedUnsynced: [mockUnsyncedWithServerId],
    });
    mockedPrismaRoutineDetailUpdate.mockRejectedValue(
      new Error("데이터베이스 에러")
    );
    mockedHandleServerError.mockReturnValue(
      NextResponse.json(
        { success: false, message: "데이터베이스 에러" },
        { status: 500 }
      )
    );

    const res = await POST(req);
    const body = await res.json();

    expect(res.status).toBe(500);
    expect(body.success).toBe(false);
    expect(body.message).toBe("데이터베이스 에러");
    expect(mockedHandleServerError).toHaveBeenCalled();
  });

  it("생성과 업데이트가 혼합된 요청을 처리한다", async () => {
    const req = new NextRequest(targetUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        mappedUnsynced: [mockUnsyncedWithServerId, mockUnsyncedWithoutServerId],
      }),
    });

    mockedValidateData.mockReturnValue({
      mappedUnsynced: [mockUnsyncedWithServerId, mockUnsyncedWithoutServerId],
    });
    mockedPrismaRoutineFindUnique.mockResolvedValue(mockRoutineResponse);
    mockedPrismaRoutineDetailUpdate.mockResolvedValue(
      createPrismaRoutineDetailResponse({ id: "updated-detail-id" })
    );
    mockedPrismaRoutineDetailCreate.mockResolvedValue(
      createPrismaRoutineDetailResponse({ id: "created-detail-id" })
    );

    const res = await POST(req);
    const body = await res.json();

    expect(res.status).toBe(200);
    expect(body.success).toBe(true);
    expect(mockedPrismaRoutineDetailCreate).toHaveBeenCalledWith({
      data: expect.objectContaining({
        routine: { connect: { id: mockUnsyncedWithoutServerId.routineId } },
        exercise: { connect: { id: mockUnsyncedWithoutServerId.exerciseId } },
      }),
    });
    expect(mockedPrismaRoutineDetailUpdate).toHaveBeenCalledWith({
      where: { id: mockUnsyncedWithServerId.serverId },
      data: expect.objectContaining({
        weight: mockUnsyncedWithServerId.weight,
      }),
    });
  });
});
